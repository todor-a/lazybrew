...
| End-to-end Homebrew | PTY-drive the built binary against real Homebrew and a temporary trusted `lazybrew/e2e` tap containing a local cask, an on-request formula, and its dependency. Run on stable Homebrew for macOS ARM and Intel, plus Homebrew `main` on a schedule. | The app starts and quits cleanly; searches and cancels safely; uninstalls the real cask; upgrades the real formula from 1.0 to 2.0; reveals the real dependency; uninstalls the formula; and removes every fixture during cleanup. |
